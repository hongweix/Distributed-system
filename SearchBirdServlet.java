 ...
 * The model is provided by SearchBirdModel.
 */
...
@WebServlet(name = "SearchBirdServlet",
        urlPatterns = {"/getAnBirdPicture"})
public class SearchBirdServlet extends HttpServlet {

    SearchBirdModel sbm = null;  // The "business model" for this app

    // Initiate this servlet by instantiating the model that it will use.
    @Override
    public void init() {
        sbm = new SearchBirdModel();
    }
    ...
    protected void doGet(HttpServletRequest request,
        ...
        if (search != null) {
             ...
             * is a String object.  Here the pictureURL, and pictureAU are passed to the view
             * after it is returned from the model interestingPictureSize method.
             */
            request.setAttribute("pictureURL",
                    sbm.interestingPictureSize(
                            (mobile) ? "mobile" : "desktop").split("\t")[0]);
            request.setAttribute("pictureAU", sbm.interestingPictureSize(
                            (mobile) ? "mobile" : "desktop").split("\t")[1]);
            // Pass the user search string (pictureTag) also to the view.
            request.setAttribute("pictureTag", sbm.getPictureTag());
